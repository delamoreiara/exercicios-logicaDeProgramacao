...
function votos(sistema) {
    var opções = [1 + '-Windows', 2 + '-Unix', 3 + '-Linux', 4 + '-Netware', 5 + '-Mac', 6 + '-Outro'];
    var totalVotos = sistema.length;
    var respostas = [];
    var count = [0, 0, 0, 0, 0, 0];
    var vencedor = "";
    var percentualMaior = 0;
    for (var i = 0; i < sistema.length; i++) {
        ...
        for (var i_1 = 0; i_1 < count.length; i_1++) {
            var percentual = (count[i_1] / totalVotos) * 100;
    ...
    return {
        ...
        count: count,
        vencedor: vencedor,
        percentualMaior: percentualMaior
    };
}
